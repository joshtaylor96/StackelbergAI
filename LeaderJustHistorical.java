 ...
 * @author Xin
 */
final class LeaderJustHistorical extends PlayerImpl {
	/**
	...
	LeaderJustHistorical() throws RemoteException, NotBoundException {
		/*
		 * The first parameter *MUST* be PlayerType.LEADER, you can change the second
		 * parameter, the name of the leader, such as "My Leader"
		 */
		super(PlayerType.LEADER, "LeaderSimple");
	...
	public void startSimulation(int p_steps) throws RemoteException {
		...
		// leaderPrices.add((float) 3.0); leaderPrices.add((float) 4.0);
		// leaderPrices.add((float) 5.0); leaderPrices.add((float) 6.0);
		// leaderPrices.add((float) 7.0);
		// followerPrices.add((float) 2.0); followerPrices.add((float) 3.0);
		// followerPrices.add((float) 3.0); followerPrices.add((float) 4.0);
		// followerPrices.add((float) 6.0);
		float aUpperFirst = 0;
		...
		a = (aUpperFirst * aUpperSecond - aUpperThird * aUpperFourth)
				/ (((float) leaderPrices.size()) * aLowerFirst - aLowerSecond * aLowerSecond);
		b = (((float) leaderPrices.size()) * aUpperFourth - aUpperThird * aUpperSecond)
				/ (((float) leaderPrices.size()) * aUpperFirst - aLowerSecond * aLowerSecond);
	...
	public void proceedNewDay(int p_date) throws RemoteException {
		/*
		 * Check for new price Record l_newRecord = m_platformStub.query(m_type,
		 * p_date);
		 *
		...
		m_platformStub.publishPrice(m_type, globalMaxCalc(a, b));
	}
	...
	private float genPrice() throws RemoteException {
		...
		float currentSales;
		float currentProfit;
		for (float currentStrat = ((float) 1.00); currentStrat < ((float) 3); currentStrat = currentStrat
				+ ((float) 0.01)) {
	...
	private float globalMaxCalc(float a, float b) {
		return (((float) 3.0 + (float) 0.3 * a - (float) 0.3 * b) / ((float) 2.0 - (float) 0.6 * b));
	}

	public static void main(final String[] p_args) throws RemoteException, NotBoundException {
		new LeaderJustHistorical();
	}
}
